...
describe('Registration', () => {
  ...
  test(`Should return error if username shorter than ${userMin} characters`, async() => {
    ...
    const type = ApiError.ValidationError.name;

    const data = await register(username, password);
    expect(data).toEqual(`${type}: Username is too short (minimum is ${userMin} characters)`);
  });
  test(`Should return error if username bigger than ${userMax} characters`, async() => {
    ...
    const data = await register(username, password);
    expect(data).toEqual(`${type}: Username is too long (maximum is ${userMax} characters)`);
  });
  test(`Should return error if password shorter than ${passMin} characters`, async() => {
    ...
    const data = await register(username, password);
    expect(data).toEqual(`${type}: Password is too short (minimum is ${passMin} characters)`);
  });
  test(`Should return error if password bigger than ${userMin} characters`, async() => {
    ...
    const data = await register(username, password);
    expect(data).toEqual(`${type}: Password is too long (maximum is ${passMax} characters)`);
  });
